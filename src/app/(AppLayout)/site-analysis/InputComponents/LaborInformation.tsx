    ...
    return (
        <InputAccordion title="Labor Information" completed={allFilled}>
            <Grid container spacing={2}>
                {/* Manager */}
                <Grid item xs={12}>
                    <Typography variant="subtitle1" fontWeight="bold" color="primary.main">Manager</Typography>
                </Grid>
                <Grid item xs={12} md={4}>{renderTextField("laborHours.manager", "Labor Hours")}</Grid>
                <Grid item xs={12} md={4}>{renderTextField("hourlyWages.manager", "Hourly Wages")}</Grid>
                <Grid item xs={12} md={4}>{renderTextField("burdenRate.manager", "Burden Rate")}</Grid>

                {/* Assistant Manager */}
                <Grid item xs={12}>
                    <Typography variant="subtitle1" fontWeight="bold" color="primary.main">Assistant Manager</Typography>
                </Grid>
                <Grid item xs={12} md={4}>{renderTextField("laborHours.assistantManager", "Labor Hours")}</Grid>
                <Grid item xs={12} md={4}>{renderTextField("hourlyWages.assistantManager", "Hourly Wages")}</Grid>
                <Grid item xs={12} md={4}>{renderTextField("burdenRate.assistantManager", "Burden Rate")}</Grid>

                {/* Attendants */}
                <Grid item xs={12}>
                    <Typography variant="subtitle1" fontWeight="bold" color="primary.main">Attendants</Typography>
                </Grid>
                <Grid item xs={12} md={3}>{renderTextField("count.attendants", "Number of Attendants")}</Grid>
                <Grid item xs={12} md={3}>{renderTextField("laborHours.attendants", "Labor Hours")}</Grid>
                <Grid item xs={12} md={3}>{renderTextField("hourlyWages.attendants", "Hourly Wages")}</Grid>
                <Grid item xs={12} md={3}>{renderTextField("burdenRate.attendants", "Burden Rate")}</Grid>
            </Grid>
        </InputAccordion>
    );
}
